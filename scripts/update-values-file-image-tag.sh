...
# 	yq -i '.global.image.tag = env(GLOBAL_IMAGE_TAG)' charts/camunda-load-tests/values.yaml
# 	yq -i '.optimize.image.tag = env(OPTIMIZE_IMAGE_TAG)' charts/camunda-load-tests/values.yaml
...
if [[ -n "${GLOBAL_IMAGE_TAG}" ]]; then
    sed -ri "s/(\s+)tag:.+# (global.image.tag)/\1tag: ${GLOBAL_IMAGE_TAG}  # \2/g" \
        charts/camunda-load-tests/values.yaml
